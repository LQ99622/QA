...
def json_to_pandas(path, gzipped=False, nrows=None):
    ...
    """
    print(2222222222222222, path)
    data = []
    if not gzipped:
        print(333333333333, path)
        with open(path) as f:
            a = 0
...
def split_answerabilty(df):
    ...
    df_unanswerable["parametric_answer"].fillna("", inplace=True)
...
def extract_context_from_annotations_for_filtered(df):
    ...
    df = df.assign(context=df.apply(lambda x: " ".join(x["document_html"].split()[x["ls"]:x["le"]]), 1))
    df = df.assign(answer=df.apply(lambda x: " ".join(x["document_html"].split()[x["ss"]:x["se"]]), 1))
...
def create_counterfactual_from_path(path_pattern, split_name="TrainTrain", sub_suffix="corpus-substitution",
                                    nrows=None):
...
def create_random_context(df_answerable):
    ...
    if len(df[df.apply(lambda x: x["answer"] in x["random_context"], axis=1)]) > 0:
        print("parametric_answer in random_context")
        # print("answer in random_context")
    df["typek"] = "random_context"
    df["contextual_answer"] = "unanswerable"
    df["answerable"] = False
    # df = df[["question", "random_context", "parametric_answer", "contextual_answer", "answerable", "type"]]
    df = df[["question", "random_context", "answer", "contextual_answer", "answerable", "type"]]
...
def enrich_nq_from_counterfactuals(path, split_name="Train", suffix="", answerability_declaration=False, nrows=None):
    ...
    df_counterfactual = create_counterfactual_from_path(path, split_name=split_name,
...
if __name__ == '__main__':
    ...
    if args.mode == "split":
        ...
        path_prefix = "./data/natural_questions/v1.0-simplified_simplified-nq-train"
        test_prefix = "./data/natural_questions/v1.0-simplified_nq-dev-all.jsonl.gz"
